/**
 * Outlook AI E2E Test Runner
 *
 * Orchestrates E2E testing by:
 * 1. Starting a custom test server to receive results from Outlook (via POST body)
 * 2. Building and serving the test add-in via Vite (port 3004)
 * 3. Sideloading into Outlook Desktop
 * 4. Opening a new compose window to trigger the LaunchEvent
 * 5. Collecting and validating test results
 *
 * The test-taskpane.ts (in Outlook) calls each Outlook tool's handler() directly
 * and reports results to this runner.
 *
 * Auto-start: The manifest uses LaunchEvent (OnNewMessageCompose) to start the
 * shared runtime automatically when a compose window opens.
 */
...
import { closeDesktopApplication, openComposeWindow } from './src/node-helpers';
...
const manifestPath = path.resolve(
  `${process.cwd()}/tests-e2e-outlook/test-manifest.xml`
);
const port = 4204;
...
function assertToolResult(name: string): void {
  ...
  assert.ok(result, `No result received for "${name}" — test may not have run in Outlook`);
  assert.strictEqual(
    result.Type,
    'pass',
...
describe('Outlook AI E2E Tests', function () {
  this.timeout(0);

  before(`Setup: start test server and sideload ${host}`, async () => {
    console.log('Setting up Outlook test environment...');
    ...
    console.log('Starting dev server and sideloading add-in into Outlook...');
    await startDebugging(manifestPath, options);
    console.log('Add-in sideloaded into Outlook');

    console.log('Opening new compose window to trigger OnNewMessageCompose LaunchEvent...');
  ...
  after('Teardown: stop server, close Outlook, unregister add-in', async () => {
    console.log('Tearing down...');
    await testServer.stop();

    console.log('Closing Outlook...');
    try {
      await closeDesktopApplication();
    } catch (error) {
      console.log(`Note: Outlook may already be closed: ${String(error)}`);
    }
  ...
  describe('Result Collection', () => {
    it('should receive test results from Outlook', () => {
  ...
  describe(`Outlook Tools (${outlookToolNames.length})`, () => {
    for (const name of outlookToolNames) {
      it(name, () => {
  ...
  describe('Summary', () => {
    it('all in-Outlook tests should pass', () => {
      const failed = e2eContext.getFailedTests();
      if (failed.length > 0) {
        const names = failed
          .map(f => `${f.Name}: ${(f.Metadata?.error as string) || 'unknown'}`)
          .join('\n  ');
        assert.fail(`${failed.length} test(s) failed in Outlook:\n  ${names}`);
      }
    });
  });
});
